...
export function Modal({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (closeOnEsc && e.key === "Escape" && isOpen) handleClose();
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          >
            {showCloseButton && (
              <button
                onClick={handleClose}
                className="absolute top-4 right-4 z-[60] p-2 rounded-full text-fg-muted hover:text-fg-base hover:bg-bg-elevated transition-all active:scale-90"
...
Modal.Header = function ModalHeader({
  ...
  return (
    <div
      className={cn(
        "px-6 py-5 border-b border-border-base shrink-0",
        className,
      )}
    >
      <h2 className="font-serif text-xl font-bold text-fg-base leading-tight">
        {children}
      </h2>
    </div>
  );
...
Modal.Body = function ModalBody({
  ...
  return (
    <div
      className={cn(
        "px-6 py-4 text-fg-base overflow-y-auto custom-scrollbar flex-1", // flex-1 allows it to grow/shrink
        className,
      )}
    >
      {children}
...
Modal.Footer = function ModalFooter({
  ...
  return (
    <div
      className={cn(
        "px-6 py-4 border-t border-border-base bg-bg-elevated/50 flex flex-col-reverse sm:flex-row justify-end gap-3 shrink-0",
        className,
      )}
    >
      {children}
...
// ... ConfirmModal follows same pattern with improved button contrast
